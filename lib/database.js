...
module.exports = {
  ...
  get_peers: function(cb) {
    Peers.find({}, function(err, peers) {
      if (err) {
        console.log('get_peers error = ' + err);
        return cb([]);
      } else {
        console.log('database.js, get_peers no error, peers = ');
        console.log(peers);
        return cb(peers);
      }
    });
   /*Address.find({}, function(err, peers) {
      console.log('inside get_peers, Address find')
      console.log(peers);
    });*/
  }
};
